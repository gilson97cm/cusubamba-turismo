@extends('backend.admin.layout')
@section('css')
    <link rel="stylesheet" href="{{asset('assets/my-libs/my-styles.css')}}">
    <link rel="stylesheet" href="{{asset('assets/my-libs/tb_products.css')}}">
@endsection
@section('breadcrumb')
    <div class="page-breadcrumb">
        <div class="row">
            <div class="col-5 align-self-center">
                <h4 class="page-title">Noticias</h4>
                <div class="d-flex align-items-center">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="#">Noticias</a></li>
                        </ol>
                    </nav>
                </div>
            </div>
        </div>
    </div>
@endsection
@section('dashboard')
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-auto col-md-12">
                <div  id="alert">
                </div>
                @include('flash::message')
            </div>
        </div>
        <div class="row ">
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <label class="my-label">  Noticias</label>
                        @can('news.create')
                            <a href="{{route('news.create')}}"
                               class="btn btn-sm btn-primary my_button pull-right ">
                                Crear
                            </a>
                        @endcan
                    </div>

                    <div class="card-body">
                        <div class="row">
                            <div class="col-lg-1"></div>
                            <div class=" form-group col-lg-10">
                                <table class="table table-striped table-hover">
                                    <thead>
                                    <tr class="thead-tr">
                                        <th width="15%">Imágen</th>
                                        <th width="35%">Titulo</th>
                                        <th width="48%">Detalles</th>
                                        <th width="2%" colspan="3">&nbsp;</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    @foreach($news as $news_)
                                        <tr>
                                            <td><img src="{{$news_->avatar_news}}" alt="img" width="80px" height="50px"></td>
                                            <td>{{$news_->title_news}}</td>
                                            <td>{!! $news_->detail_news !!}</td>
                                            <td width="10px">
                                                @can('news.show')
                                                    <a href="{{route('news.show', $news_->id_news)}}"
                                                       class="btn btn-sm btn-primary my_button">
                                                        Ver
                                                    </a>
                                                @endcan
                                            </td>
                                            <td width="10px">
                                                @can('news.edit')
                                                    <a href="{{route('news.edit', $news_->id_news)}}"
                                                       class="btn btn-sm btn-secondary">
                                                        Editar
                                                    </a>
                                                @endcan
                                            </td>
                                            <td width="10px">
                                                @can('news.destroy')
                                                    {!! Form::open(['route' => ['news.destroy', $news_->id], 'method' => 'DELETE']) !!}
                                                    <a href="#" disabled="true"  class="btn btn-sm btn-danger btn-destroy">Eliminar</a>
                                                    {!! Form::close() !!}
                                                @endcan
                                            </td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
                                {!! $news->render() !!}
                            </div>
                            <div class="col-lg-1"></div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
@section('scripts')
    <script src="{{asset('assets/my-libs/js/crud_role.js')}}"></script>
    <script src="{{asset('assets/libs/sweetalert2/dist/sweetalert2.all.min.js')}}"></script>
    <script>
        $('div.alert').not('.alert-important').delay(2000).fadeOut(4000);
    </script>
@endsection
